...
public final class RedisSinks {
    ...
    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Hash. Assumes that all keys and values
     * are {@link String}s and that incoming data items are raw {@link Map.Entry}s.
     *
    ...
    public static Sink<Map.Entry<String, String>> hash(
        ...
        return hash(name, RedisURI.create(uriString), hash, StringCodec::new, Map.Entry::getKey, Map.Entry::getValue);
    }
     ...
     * @param uriString URI of the Redis server in String form
     * @param hash      identifier of the Redis Hash being used
     * @param keyFn     function that specifies how to extract the hash key from incoming data items
     * @param valueFn   function that specifies how to extract the hash value from incoming data items
    ...
    public static <T> Sink<T> hash(
            ...
            FunctionEx<T, String> valueFn
    ) {
        return hash(name, RedisURI.create(uriString), hash, StringCodec::new, keyFn, valueFn);
    }
    ...
    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Hash.
     *
     * @param name      name of the source being created
     * @param uriString URI of the Redis server in String form
    ...
    public static <K, V, T> Sink<T> hash(
            ...
            FunctionEx<T, K> keyFn,
            FunctionEx<T, V> valueFn
    ) {
        return hash(name, RedisURI.create(uriString), hash, codecFn, keyFn, valueFn);
    }
    ...
    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Sorted Set. Assumes that the sorted
     * set identifier and stored values are all {@code String}s and the incoming data items are
     * {@link ScoredValue<String>}s.
     *
     * @param name      name of the source being created
     * @param uriString URI of the Redis server in String form
    ...
    public static Sink<ScoredValue<String>> sortedSet(
        ...
        return sortedSet(name, RedisURI.create(uriString), StringCodec::new, set, ScoredValue::getScore,
    ...
    public static Sink<ScoredValue<String>> sortedSet(
        ...
        return sortedSet(name, uri, StringCodec::new, set, ScoredValue::getScore, ScoredValue::getValue);
    }

    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Sorted Set. Assumes that the sorted
     * set identifier and stored values are all {@code String}s.
     *
     * @param name      name of the source being created
     * @param uriString URI of the Redis server in String form
     * @param set       identifier of the Redis Sorted Set being used
     * @param scoreFn   function that specifies how to extract the scores from incoming data items
     * @param valueFn   function that specifies how to extract stored values from incoming data items
    ...
    public static <T> Sink<T> sortedSet(
            ...
            String uriString,
            String set,
            FunctionEx<T, Double> scoreFn,
            FunctionEx<T, String> valueFn
    ) {
        return sortedSet(name, RedisURI.create(uriString), StringCodec::new, set, scoreFn, valueFn);
    }
    ...
    public static <T> Sink<T> sortedSet(
        ...
        return sortedSet(name, uri, StringCodec::new, set, scoreFn, valueFn);
    }

    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Sorted Set.
     *
     * @param name      name of the source being created
     * @param uriString URI of the Redis server in String form
     * @param codecFn   supplier of {@link RedisCodec} instances, used in turn for serializing/deserializing
    ...
    public static <K, V, T> Sink<T> sortedSet(
        ...
        return sortedSet(name, RedisURI.create(uriString), codecFn, set, scoreFn, valueFn);
    }
    ...
    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Stream. Assumes all keys and values
     * are {@code String}s. Uses a very simple mapping function which translates incoming data items directly to field
     * values (via their {@code toString()} method).
     *
    ...
    public static <T> Sink<T> stream(
        ...
        return stream(name, RedisURI.create(uriString), stream, StringCodec::new, mapFn);
    }
     ...
     * @param uriString URI of the Redis server in String form
     * @param stream    identifier of stream being used
     * @param mapFn     mapping function used to transform an incoming element into a Redis Stream entry's body
     * @return sink to use in {@link com.hazelcast.jet.pipeline.Pipeline#drainTo}
     */
    public static <T> Sink<T> stream(
            String name,
            String uriString,
            String stream,
            FunctionEx<T, Map<String, String>> mapFn
    ) {
        return stream(name, RedisURI.create(uriString), stream, StringCodec::new, mapFn);
    }
    ...
    /**
     * Creates a {@link Sink} which pushes data items into a specified Redis Stream.
     *
     * @param <T>       type of elements forming the input of this sink
    ...
    public static <T, K, V> Sink<T> stream(
        ...
        return stream(name, RedisURI.create(uriString), stream, codecFn, mapFn);
    }
